...
import (
	...
	"github.com/d-demirci/Project-Whis/C2/core"
...
func main() {
	...
	if core.Err != nil {
		fmt.Println("[!] CHECK MYSQL SETTINGS IN 'config.toml' ! [!]")
	...
	if core.Err != nil {
		fmt.Println("[!] CHECK IF MYSQL SERVER IS ONLINE! [!]")
		core.Log.Fatalln("[!] CHECK IF MYSQL SERVER IS ONLINE! [!]")
	}
	fmt.Println("SQL Connection Good...")
