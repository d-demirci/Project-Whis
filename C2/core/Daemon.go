...
func Daemon() { //Handle background stuff like counts and filters
	for {
		...
		rows, err := DB.Query("SELECT UID, LastResponse FROM windows_clients")
		if err != nil {
			// log.Print("[!] SELECT UID, LastResponse FROM windows_clients [!]")
		} else {
			defer rows.Close()
			for rows.Next() {
		...
		if err != nil {
			// log.Print("[!] SELECT PasswordCount FROM windows_clients [!]")
		} else {
			defer rows.Close()
			for rows.Next() {
